                    ...
                    <form action="{{ route('admin.families.update', $family->id) }}" class="modal_style" id="myForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group mb-3 col-md-3 mother_widows_support_amount @if (old('mother_widows_support', $family->mother_widows_support) != '1') hidden @endif">
                                <x-inputs.text name="mother_widows_support_amount" label="مبلغ الدعم للأم" :value="old('mother_widows_support_amount', $family->mother_widows_support_amount)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />
                            </div>
                            <div class="form-group mb-3 col-md-3">
                            ...
                            <div class="form-group mb-3 col-md-3 husband_retirement_compensation_amount  @if (old('has_retirement_compensation', $family->has_retirement_compensation) != '1') hidden @endif">
                                <x-inputs.text name="husband_retirement_compensation_amount" label="المبلغ الشهري من تعويض تقاعد الزوج" :value="old('husband_retirement_compensation_amount', $family->husband_retirement_compensation_amount)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />
                            </div>
                            <div class="form-group mb-3 col-md-3">
                            ...
                            <div class="form-group mb-3 col-md-3 other_mother_income @if (old('is_there_another_source_of_income', $family->is_there_another_source_of_income) != '1') hidden @endif">
                                <x-inputs.text name="mother_other_income_amount" label="المبلغ الشهري للدخل الأخر" :value="old('mother_other_income_amount', $family->mother_other_income_amount)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />
                            </div>
                            <div class="form-group mb-3 col-md-3 other_mother_income @if (old('is_there_another_source_of_income', $family->is_there_another_source_of_income) != '1') hidden @endif">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group mb-3 col-md-3 housing_rent @if (old('housing_ownership', $family->housing_ownership) != '1') hidden @endif">
                                <x-inputs.text name="rent_amount" label="القيمة الإيجارية" :value="old('rent_amount', $family->rent_amount)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />
                            </div>
                            <div class="form-group mb-3 col-md-3">
                            ...
                            <div class="form-group mb-3 col-md-3">
                                <label class="form-label" for="has_water">هل يتوفر ماء ؟</label>
                                <select name="has_water" id="has_water" class="form-control">
                                    <option disabled>اختر من القائمة...</option>
                                    @foreach (config('options.boolean') as $key => $label)
                                        <option value="{{ $key }}" @if (old('has_water', $family->has_water) == $key) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('has_water')
                                    <span class="text-danger">{{ $message }}</span>
                            ...
                            <div class="form-group mb-3 col-md-3">
                                <label class="form-label" for="has_electricity">هل تتوفر كهرباء ؟</label>
                                <select name="has_electricity" id="has_electricity" class="form-control">
                                    <option disabled>اختر من القائمة...</option>
                                    @foreach (config('options.boolean') as $key => $label)
                                        <option value="{{ $key }}" @if (old('has_electricity', $family->has_electricity) == $key) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-3 col-md-3">
                                <label class="form-label" for="has_sewage">هل يتوفر صرف صحي ؟</label>
                                <select name="has_sewage" id="has_sewage" class="form-control">
                                    <option disabled>اختر من القائمة...</option>
                                    @foreach (config('options.boolean') as $key => $label)
                                        <option value="{{ $key }}" @if (old('has_sewage', $family->has_sewage) == $key) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('has_sewage')
                                    <span class="text-danger">{{ $message }}</span>
...
@push('js')
    ...
    <script>
        ...
        // housing ownership
        $(document).on('change', '#housing_ownership', function(e) {
            if ($(this).val() == '1') {
                $(".housing_rent").show();
            } else {
                $(".housing_rent").hide();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#attachment-row-' + attachmentId).fadeOut(500, function() {
                                $(this).remove();
                            });
            ...
            $('#myForm').validate({
                ...
                rules: {
                    ...
                    "mother_widows_support_amount": {
                        requiredIf: {
                            field: "#does_mother_take_widows_support",
                            value: "1"
                        }
                    },
                    ...
                    "husband_retirement_compensation_amount": {
                        requiredIf: {
                            field: "#has_retirement_compensation",
                            value: "1"
                        }
                    },
                    ...
                    "mother_other_income_type": {
                        requiredIf: {
                            field: "#is_there_another_source_of_income",
                            value: "1"
                        }
                    },
