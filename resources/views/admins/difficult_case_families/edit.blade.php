  ...
  <div class="col-xl-12 mb-30">
    <div class="card card-statistics h-100">
      <div class="card-body">
        ...
        @endif
        <form action="{{ route('admin.difficult-case-families.update', $difficultCaseFamily->id) }}" class="modal_style" id="difficultCaseFamilyForm" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="row">
            ...
            <div class="form-group mb-3 col-md-3">
              ...
              <select name="gender" id="gender" class="form-control">
                ...
                <option value="{{ $key }}" @if(old('gender', $difficultCaseFamily->gender) == $key) selected @endif>{{ $label }}</option>
                @endforeach
              </select>
            ...
            <div class="form-group mb-3 col-md-3">
              <label class="form-label" for="education_level">المستوى الدراسي</label>
              <select name="education_level" id="education_level" class="form-control">
                <option disabled>اختر من القائمة...</option>
                @foreach(config('options.education_level') as $key => $label)
                <option value="{{ $key }}" @if(old('education_level', $difficultCaseFamily->education_level) == $key) selected @endif>{{ $label }}</option>
                @endforeach
              </select>
            ...
            <div class="form-group mb-3 col-md-3">
                ...
                <select name="family_members_count">
                  ...
                  <option value="{{ $key }}" @if (old('family_members_count', $difficultCaseFamily->family_members_count)==$key) selected @endif>{{ $label }}</option>
                  @endforeach
                </select>
            ...
            <div class="form-group mb-3 col-md-3">
              <label class="form-label" for="difficult_case_type">فئة الحالة</label>
              <select name="difficult_case_type" id="difficult_case_type" class="form-control">
                ...
                <option value="{{ $key }}" @if(old('difficult_case_type', $difficultCaseFamily->difficult_case_type) == $key) selected @endif>{{ $label }}</option>
                @endforeach
              </select>
              @error('difficult_case_type')<span class="text-danger">{{ $message }}</span>@enderror
            </div>
            <div class="form-group mb-3 col-md-3">
              ...
              <select name="social_status" id="social_status" class="form-control">
                ...
                <option value="{{ $key }}" @if(old('social_status', $difficultCaseFamily->social_status) == $key) selected @endif>{{ $label }}</option>
                @endforeach
              </select>
      ...
      $('#difficultCaseFamilyForm').validate({
        ignore: [],
          rules: { 
            ...
            first_name_ar: { required: true }, last_name_ar: { required: true }, first_name_fr: { required: true }, last_name_fr: { required: true }, national_id_no: { required: true }, gender: { required: true }, birth_date: { required: true }, education_level: { required: true }, family_members_count: { required: true  }, difficult_case_type: { required: true }, social_status: { required: true }, governorate_id: { required: true }, city_id: { required: true }, address: { required: true }, phone: { required: true }, },
          messages :{ registration_date: { required: 'الحقل مطلوب' }, first_name_ar: { required: 'الحقل مطلوب' }, last_name_ar: { required: 'الحقل مطلوب' }, first_name_fr: { required: 'الحقل مطلوب' }, last_name_fr: { required: 'الحقل مطلوب' }, national_id_no: { required: 'الحقل مطلوب' }, gender: { required: 'الحقل مطلوب' }, birth_date: { required: 'الحقل مطلوب' }, education_level: { required: 'الحقل مطلوب' }, family_members_count: { required: 'الحقل مطلوب', min: 'الحقل يجب ان يكون على الاقل 1', max: 'الحقل يجب ان يكون على الاقل 50' }, difficult_case_type: { required: 'الحقل مطلوب' }, social_status: { required: 'الحقل مطلوب' }, governorate_id: { required: 'الحقل مطلوب' }, city_id: { required: 'الحقل مطلوب' }, address: { required: 'الحقل مطلوب' }, phone: { required: 'الحقل مطلوب' }, },
          errorElement : 'span', 
...
<script>
  $(document).ready(function() {
      ...
      if (initialGovernorateId) {
          var cityIdToSelect = "{{ old('city_id', $difficultCaseFamily->city_id) }}";
