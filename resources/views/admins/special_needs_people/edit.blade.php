                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.special-needs-people.update', $specialNeedsPerson->id) }}" class="modal_style" id="specialNeedsPersonForm" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group mb-3 col-md-3">
                                ...
                                <select name="service_type" class="form-control">
                                    ...
                                    @foreach (config('options.service_type') as $key => $label)
                                        <option value="{{ $key }}" @if (old('service_type', $specialNeedsPerson->service_type) == $key) selected @endif>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {{-- // add social  category field --}}
                            <div class="form-group mb-3 col-md-3">
                               <label class="form-label">الحالة الاجتماعية</label>
                                <select name="social_category" class="form-control">
                                    <option disabled>اختر من القائمة...</option>
                                    @foreach (config('options.social_category') as $key => $label)
                                        <option value="{{ $key }}" @if (old('social_category', $specialNeedsPerson->social_category) == $key) selected @endif>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        $(document).ready(function() {
            $('#difficultCaseFamilyForm').validate({
                ...
                messages: {
                    ...
                    family_members_count: {
                        required: 'الحقل مطلوب',
                        min: 'الحقل يجب ان يكون على الاقل 1',
                        max: 'الحقل يجب ان يكون على الاقل 50'
                    },
                    special_needs_type: {
                        required: 'الحقل مطلوب'
                    },
